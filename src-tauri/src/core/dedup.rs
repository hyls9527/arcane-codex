    ...
    pub fn scan(&self, db: &Database) -> AppResult<ScanResult> {
        ...
        for i in 0..images.len() {
            for j in (i + 1)..images.len() {
                let distance = ImageProcessor::hamming_distance(&images[i].6, &images[j].6)?;
    ...
    fn cluster_duplicates(
        ...
        for &(i, j, _distance) in pairs {
            ...
            for &cluster_id in cluster_map.keys() {
                if cluster_map[&cluster_id].contains(&i) || cluster_map[&cluster_id].contains(&j) {
                    found_cluster = Some(cluster_id);
                    break;
                }
            }

            if let Some(cluster_id) = found_cluster {
                if !cluster_map[&cluster_id].contains(&i) {
                    cluster_map.get_mut(&cluster_id).unwrap().push(i);
                }
                if !cluster_map[&cluster_id].contains(&j) {
                    cluster_map.get_mut(&cluster_id).unwrap().push(j);
                }
            } else {
                let new_id = cluster_map.len();
                cluster_map.insert(new_id, vec![i, j]);
            }
        }
